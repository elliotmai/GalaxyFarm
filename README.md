...
**A redirect is never filed under the page it came from.** Only a plain 200 fetched from the URL it was asked for goes into the app-shell cache — `mayCacheDocument` in `lib/sw-contract.ts`. Without that rule the reply to a sign-in or pairing redirect gets stored under a board's own URL, and from then on every navigation that falls back to the cache renders a page asking the screen to sign in, on a device whose session is fine. Four slow seconds is enough to trigger the fallback, and a page that waits up to eight for a Neon cold start can spend them on an ordinary morning. It would never age out, either — the cache measures age from last use, so being served is what keeps it fresh.
...
**That promise is only as good as the surface a screen is standing on, and `/login` used to break it.** "Applies it itself" is a property of the kiosk layout — it is the one surface that passes `unattended` — so a barn screen that ended up anywhere else got the tap-to-update bar instead, on a wall-mounted tablet with nobody to tap it. Every later build then installed, waited, and was never applied: the screen was pinned to that build for good, and no fix shipped afterwards could reach it. `/login` was where a screen with a lapsed session landed, which made it the one page in the app where being stranded was likely rather than theoretical. It now redirects a kiosk-bound visitor to `/kiosk/pair` — on the kiosk surface, where the wait ends by itself — and `?as=person` is the way through for the owner on a laptop who followed a bookmark to a board.
...
**The screen keeps that token, and that is what makes pairing a once-ever job.** It lives in an `httpOnly` cookie the page's own script cannot read, so an XSS on a board cannot walk off with it — and it outlives the Auth.js session, which is an ordinary thirty-day JWT like anybody's login. When that session goes, for whatever reason took it, a signed-out board is sent to `/kiosk/pair` and never to `/login`: a login form is a dead end on a wall-mounted tablet with no keyboard and no account behind it, and every lapse used to cost somebody a walk out with a fresh code. Holding the token means the screen signs itself back in instead, in the second before anyone notices. It widens nothing — `authenticateDevice` refuses a revoked row, so a resume fails exactly where a sign-in would — and a screen that signs in successfully and finds itself signed out again three times inside a minute stops rather than looping. Losing signal is told apart from being revoked, because one should keep retrying by itself and the other should ask for a code.

**A screen is a record as well as a credential.** §4.5's exception list exempts a _pairing token_ — "revocable, not editable" — and the row carrying it is an ordinary record that gets the ordinary surface: rename it when the tablet moves from the tack room to the coop, delete it when the screen is gone for good. Delete is a tombstone rather than a `DELETE`, and it stops the screen by exactly the mechanism revoking does — `authenticateDevice`, `redeemPairing` and the live check all pass over a tombstoned row, so a deleted board loses its session and its pull within one sync interval. What it does not do is revoke, and that is the point of clause 4: restore the row from Settings and a screen that was working goes on working, without anybody walking a fresh code out to the barn. Revoking stays the separate, blunter thing — it says the screen has stopped, and leaves the row in the list saying so.
...
**The Pen Board is the heart of the app.** The property map with live animal positions, merged care instructions, halter-color swatches, and safety-level color coding is what gets glanced at ten times a day and what barn screens show by default. Pets are not on it — a dog has no pen (§5.8), so it draws no chip, fills no pen, and cannot be dragged into one; the housesitter guide briefs it in the pets' own section.
