...
**Zone** — the universal "place": `type: pen | pasture | coop | barn | stall | garden_area | working_facility | off_site | area` (`working_facility` added v1.3 — the tub and chute hold cattle under handling but nothing lives there, so they must not render on the Pen Board as though something did; `area` added v1.5 for the same reason in reverse — North and South *hold* pens rather than being one), name, **parentZoneId** (added v1.5 — the area or barn this sits in), `indoor/outdoor`, capacity, polygon geometry for the map, **baseline safetyLevel** (hazards of the place itself — electric fence, footing, equipment; see *Safety levels* below), **waterSourceIds** referencing **WaterSource** records (added v1.3 — see below; feeds the freeze alerts in §6), **customInstructions** (rich text — group-level care instructions live here), active flag. Nine zones seeded — **Pasture, Hay Field, West Pen, Pen 1, 2nd Pen, Pen A, Pen B, Tub/Chute, and Randy's pasture** (the neighbour's, used on and off); see `docs/property-layout.md`. Fully editable as the layout changes. **Zones group into one another through `parentZoneId`** (added v1.5): stalls are in barns, and pens and pastures are in areas — the same sentence twice, so one field rather than a parent for barns and a separate group name for areas. The group is a Zone, not a string on each pen, because a string gets retyped until "North", "north" and "Norht" are three groups nobody meant; and because a barn is then picked **by name** — a stall is in the Red Barn, not in "a barn". `canContain` fixes the legal pairings (no area inside an area, no stall loose in a field) and `possibleGroupsFor` additionally refuses to put a zone inside its own descendant. **A zone with no parent is its own group**, which is the ordinary state, not a gap. `indoor` decides which of an animal's two slots a zone fills (§5.1 *Zone assignment*): one inside and one outside at once, because a horse has a stall and a pasture both. Note three open modelling questions recorded there: tanks are shared between zones, one tank is seasonal, and the tub needs a zone type that is not `pen`.
...
Services are **grouped into attempts**, derived rather than stored (§2): a cow that comes back open is bred again — often to a different bull, sometimes by a different method — and what the log answers is not "what happened on 14 February" but "what it took to get her in calf". An attempt ends at a calving, or when more than a full gestation has passed since the last service. Only the service that still stands projects anything: one due date per attempt, not one per service, and a re-breeding retires the window, the preg check and the watch of the service it answers.

**CalvingRecord** — dam, **the breeding it answers**, date, birthWeight, calvingEase (1–5), vigor, notes, assist details; **creates the calf as a new Animal** with pedigree pre-wired to dam + service sire. The link runs both ways: the breeding log shows the calf that came of each service and the actual gestation against the projection, one service answers one calving, and a service she has answered leaves the calving watch.
...
**A pet has no zone.** A pen is a fact about grazing, handling and gates; a dog lives in the house and goes where the family goes, so "which pasture is the cat in" is a question with no answer worth storing. Pets are therefore absent from everything that joins an animal to ground — no chip on the property map and none in its tray of animals off the map, no line on the Pen Board, no head against a pen's capacity, no contribution to a zone's effective safety level, and no share of a zone feeding plan. The housesitter guide briefs them in its own pet section instead, which is where §5.10's reader looks for them. Placements naming a pet — an accidental drag on the map wrote them before this rule existed — are ignored wherever occupancy is worked out rather than deleted, because history is append-only (§5.1).
...
**Trip** — when the owners are away, and how to reach them before they are back: name, destination, `startDate`/`endDate`, who is away, a number to try, notes, and the journey as `legs` (transport, service number, from, to, departure and arrival). The guide says how the place is run; this answers the question a sitter asks first and the guide never addresses. Stored rather than derived — nothing else in the farm's records knows a trip is happening. `awayWindow` turns the dates into the "back Sunday" a kiosk leads with and `currentTrip` picks the one trip a sitter's screen should be about (whoever is away now, else whoever leaves soonest), so no helper is ever asked to choose from a list.

*Two storage decisions that look inconsistent and are not.* `startDate`/`endDate` are real timestamps because they are compared against today on every render. The times inside `legs` are **wall clocks with an IANA zone beside them** — a departure is a clock face in a place ("nine twenty-one, Central"), read aloud to a sitter and never compared to anything, so converting it to an instant on the way in and back on the way out would need a zone database twice, could be wrong across a DST boundary, and would buy nothing. `legs` is one jsonb field rather than a child table for the reason `includes` is `text[]`: sync patches *fields* (§4.2), and a child table has no field-level patch.
...
- **Calving watch:** for every cow inside her calving window (due date ± 14 days, configurable) **who has not yet calved** — the calving closes the breeding out, so a cow with a calf at side leaves the watch that day rather than sitting on it for the rest of the fortnight — the dashboard shows a watch card and notifications fire when the window coincides with a **full moon** (± 1 day — computed locally with an astronomy library, so moon phases render on the calendar indefinitely and offline), a **rapid barometric fall** (default ≥ 4 hPa / ~0.12 inHg within 24 h) or forecast low-pressure trough, or a **cold snap** (calf-chill threshold, default 20 °F). Example alert: *"Front arriving Thursday night + full moon Friday — Dolly is at day 279."*
...
/admin/cattle/catalog            search the crawled association herdbooks; bring an animal across
...
**Daylight is the look; night is a working mode.** `flying-day` is what the farm looks like: registration certificates, sale catalogues and herd ledgers are daylight objects, and so are the jobs — printing a pedigree, showing a buyer a screen, reading a kiosk in a sunlit barn.

Night is not a second look, it is a mode for people doing chores in the dark, so the surfaces divide by what happens on them rather than by preference:

| Surface | Theme | Why |
| --- | --- | --- |
| `/admin`, `/kiosk`, `/sitter` | `flying-auto` | Work happens here at every hour. A barn screen at four in the morning during calving is the one place a white page is not neutral but a torch in the face. Follows the device — nobody sets a preference for a farm app, they set one for their phone and then open whatever they open. |
| `/account`, `/book`, public pages | `flying-day` | The farm presented rather than worked. These get printed and shown to buyers, and whoever arrives at the front door should get the one impression rather than whichever of two their laptop was set to. |

`flying-auto` shares the `flying-day` block and swaps to the `flying-night` values under `prefers-color-scheme: dark`. A media query rather than `light-dark()`: on a browser that does not know that function the whole declaration is invalid, which leaves the token unset and paints the surface transparent rather than falling back — and a barn kiosk is exactly the machine still running an old browser. The night values are therefore written twice, and `theme-tokens.test.ts` compares the two copies so they cannot drift.

The three following surfaces set their own `themeColor`; chrome that stayed light above a dark page is a bar of the wrong colour across the top of every screen.
...
34. **Pedigrees are a certificate bracket, not a constellation** (§8 v0.9). The star chart was drawn for Midnight Nebula and its own docstring said so. Rendered on the new ground three things failed at once: the halo became the "smudge on cream" it predicted for a light theme; its stated reason for existing — drawing unknown ancestors as unnamed stars, so a gap "looks like something rather than like nothing" — was never implemented, since `place` returned early on `undefined` and left the slot blank; and a repeated ancestor was marked by switching to the identity colour, which in this palette is the same navy as the action colour, so line breeding became undetectable. The bracket draws every slot including the empty ones, quietly, and marks a repeat with the word.
35. **A type scale in `em`, so hierarchy follows the surface** (§8 v0.9). Headings were Tailwind's fixed rem steps chosen per screen, which do not move with density: the kiosk ran 20px body text under the same 24px heading a laptop got at 15px, so the hierarchy grew *weaker* as the screen got further away. Sizes are now `em` on `h1`–`h4`, measured against the density root, with a minor third between the steps — and in `@layer base`, so a screen that genuinely needs its own size still wins with a utility.
36. **The explanation is folded, not deleted** (§8 v0.9). `subtitle` and `description` are both documented as one line and were both being handed paragraphs — 199 of them, median 116 characters — so Supplies opened with three lines about how a running total is computed before a single supply appeared. None of it is padding, and a farm app that hides why a number moved is worse than a wordy one; but it is read once and competes with the records every day after. The first sentence stays, the rest goes behind a disclosure, and the whole text prints.
37. **Night follows the device on the working surfaces only** (§8 v0.9). `flying-night` had tokens, a contrast suite and an entry in this document, and nothing anywhere selected it. `flying-auto` selects it, on `/admin`, `/kiosk` and `/sitter`; `/account` and the public pages stay in daylight because they are the farm presented rather than worked, and get printed. A media query rather than `light-dark()`, because an unknown function leaves the token unset and paints the surface transparent rather than falling back — and a barn kiosk is the machine still running an old browser.

**v1.6 additions:**

38. **A pet has no zone** (§5.8). Reported from the field: a dog was dragged into a pen on the property map. The map offered the move because it drew him — a pet has no pen, so his chip sat in the tray of animals not on the map, in among the stock and one gesture from the North Trap. A pen is a fact about grazing, handling and gates, and a dog goes where the family goes, so the placement had nothing true to record. Pets are now absent from every join between an animal and ground: no chip on the map or in its tray, no line on the Pen Board, no head against a pen's capacity, no contribution to a zone's effective safety level, no share of a zone feeding plan. The guide keeps briefing them in the pets' own section, which is where §5.10's reader looks. The filter is on **placements, not animals**, and it is read-side: rows an earlier mis-drag wrote stop counting rather than being deleted, so §5.1's append-only history stays intact and no migration is needed.
